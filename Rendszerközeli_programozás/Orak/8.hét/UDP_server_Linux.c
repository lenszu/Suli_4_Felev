...
int main(int argc, char *argv[]){
   ...
   server.sin_addr.s_addr = INADDR_ANY; //szerver vagyok, a címem akármi lehet
   server.sin_port        = htons(PORT_NO); //hanyas porton figyeljen
   server_size = sizeof server;  //mennyi helyet foglal a server
   client_size = sizeof client; //mennyi helyet foglal a client
   signal(SIGINT, stop); //SIGINT jelzésre leáll a szerver
   signal(SIGTERM, stop); //SIGTERM jelzésre leáll a szerver
